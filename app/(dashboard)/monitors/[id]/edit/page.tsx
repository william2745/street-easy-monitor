...
export default function EditMonitorPage() {
  ...
  if (fetching) {
    return (
      <div className="max-w-2xl">
        <div className="mb-6">
          <div className="h-6 w-48 bg-warm-300 rounded animate-pulse" />
        </div>
        <div className="space-y-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="bg-warm-50 rounded-lg border border-warm-400 p-5 h-28 animate-pulse" />
          ))}
        </div>
      </div>
    )
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        <Link href={`/monitors/${id}`} className="text-xs text-warm-600 hover:text-brand transition-colors mb-2 inline-flex items-center gap-1">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="15 18 9 12 15 6" />
          </svg>
          Back
        </Link>
        <h1 className="text-xl font-semibold text-warm-900">Edit monitor</h1>
        <p className="text-sm text-warm-600 mt-0.5">Update your criteria. Existing matches are preserved.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <section className="bg-warm-50 rounded-lg border border-warm-400 p-5">
          ...
          <input
            ...
            className="w-full border border-warm-400 rounded-lg px-3 py-2.5 text-sm text-warm-900 bg-warm-50 placeholder:text-warm-600 focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand transition-colors"
          />
        </section>
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Link href={`/monitors/${id}`} className="text-sm text-warm-600 hover:text-warm-800 transition-colors">
            Cancel
          </Link>
        </div>
      </form>
